...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
        $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="pages/main.html"><i class="layui-icon" data-icon=""></i><cite>Homepage</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				if($(".layui-tab-title.top_tab li").length == openTabNum){
					layer.msg('You can only have'+openTabNum+'TAB open at the same time！');
					return;
				}
				tabIdIndex++;
	...
	$(".refresh").on("click",function(){  //此处添加禁止连续点击刷新一是为了降低服务器压力，另外一个就是为了防止超快点击造成chrome本身的一些js文件的报错(不过貌似这个问题还是存在，不过概率小了很多)
		if($(this).hasClass("refreshThis")){
			...
			layer.msg("You are clicking faster,  please wait two seconds to refresh！");
		}
	})

	//关闭其他
	$(".closePageOther").on("click",function(){
        if ($("#top_tabs li").length > 2 && $("#top_tabs li.layui-this cite").text() !="Homepage"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
        ...
        } else if ($("#top_tabs li.layui-this cite").text() =="Homepage" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
			...
			layer.msg("No more Windows to close");
		}
		//渲染顶部窗口
		tab.tabMove();
	})
